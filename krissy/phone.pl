#!/usr/bin/perl -I . -I /www/port80/root/elect/krissy
push(@INC,".","/www/port80/root/elect/krissy");
use strict;
...
sub	getCmpid {
	...
	my $sql = "select cmpid from phone where phid=$phid";
...
sub	getPhoneList	{
	...
	my $sql = "select phonelist from phone where phid=$phid";
	my $sth = $conn->exec($sql);
	my $msg .= $conn->errorMessage;
	my $phonelist;
	if($sth) {
...
#		my $sql = "select count(distinct($phonetbl.voter_id)) from $phonetbl p1, $phonetbl p2, mvf where (p1.stat=$pstatid) and p2.voter_id not in ( select distinct(voter_id) from av where av.date_returned is not null) and (p1.voter_id=p2.voter_id and p1.voter_id=mvf.voter_id) ";
	...
	my $order = "desc";
	if($cnt) {
		if(($phonetbl eq "krissy_abs_latino") ||
		   ($phonetbl eq "krissy_eday_latino") ||
		   ($phonetbl eq "krissy_cn_abs") ||
		   ($phonetbl eq "krissy_senior_abs") ||
		   ($phonetbl eq "krissy_senior_eday") ||
		   ($phonetbl eq "krissy_oct_regs") ||
		   ($phonetbl eq "krissy_rus_abs")) {
			$order = "asc";
...
<HEAD>
$to
<TITLE>Krissy Keefer for Congress -- Phonebank</TITLE>
</HEAD>
<BODY OnLoad="timeIt()">
<form name="timerform">
...
<FORM ACTION='http://cybre.net/elect/krissy/phone.pl' METHOD='GET'>
...
my $sql = "select distinct(phid),phonelist,title,phone.cmpid,campaign from phone,cmp where phone.cmpid=cmp.cmpid and cmp.cmpid=$CMP";
my $sth = $conn->exec($sql);
...
print << "EOF";
<H2>Krissy Keefer for Congress -- Phonelists</H2>
$msg
<br>
$emsg
<br>
