#!/usr/bin/perl -I . -I /www/port80/root/elect/chris
push(@INC,".","/www/port80/root/elect/chris");
use strict;
...
sub	getCmpid {
	...
	my $sql = "select cmpid from lists phone where id=$phid";
...
sub	getPhoneList	{
	...
	my $sql = "select tag from lists phone where id=$phid";
	my $sth = $conn->exec($sql);
	my $msg .= $conn->errorMessage;
	my $phonelist;
	if($sth) {
...
sub	getOutreachTypeID	{
	my($ort) = shift;

	my $sql = "select ortid from outreachtypes where type='PHONE' and feedback='t'";
	my $sth = $conn->exec($sql);
	$msg .= $conn->errorMessage;
	my $id;
	if($sth) {
...
sub	clearLOK	{
	...
	my $unl = getpstatID('UNL');
	my $t = time;
	my $sql = "update $phonetbl set stat=$unl where stat=$lok and $t-stamp > $TIMEOUT";
	my $sth = $conn->exec($sql);
}
...
sub	doCanv	{
	...
	my $phid = $Elts{phid};
	my $out = &getOutreachTypeID();
	$notes =~ s/\'/\\'/g;
	$cmpid=$Elts{cmpid};
	my $sql = "insert into canv (sup,sgn,vol,msg,bak,na,bldg,hpty,cmpid,notes,usr,ip,stamp,voter_id,out,outid,fb) values ($idtype,$sgn,$vol,$msg,$bak,$na,$bldg,$hpty,$cmpid,\'$notes\',\'$ENV{REMOTE_USER}\',\'$ENV{REMOTE_ADDR}\',now(),$voter_id,$out,$phid,2)";
	my $sth = $conn->exec($sql);
	my $msg .= $conn->errorMessage;
}
...
sub	phone {
	...
	$phid = $Elts{phid};
	my $ok = 0;
	my $sql = "select campaign,tag,cmp.cmpid from lists phone,cmp where phone.cmpid=cmp.cmpid and id=$phid";
	my $sth = $conn->exec($sql);
	my(@row) = $sth->fetchrow;
	my $campaign = $row[0];
	my $phonelist = $row[1];
	my $cmpid = $row[2];
	my $phonetbl = $campaign."_".$phonelist;
	&lockTable($phonetbl);
...
#		my $sql = "select count(distinct($phonetbl.voter_id)) from $phonetbl p1, $phonetbl p2, mvf where (p1.stat=$pstatid) and p2.voter_id not in ( select distinct(voter_id) from av where av.date_returned is not null) and (p1.voter_id=p2.voter_id and p1.voter_id=mvf.voter_id) ";
	...
	my $order = "desc";
	if($cnt) {
		if(($phonetbl eq "chris_abs_latino") ||
		   ($phonetbl eq "chris_eday_latino") ||
		   ($phonetbl eq "chris_cn_abs") ||
		   ($phonetbl eq "chris_senior_abs") ||
		   ($phonetbl eq "chris_senior_eday") ||
		   ($phonetbl eq "chris_oct_regs") ||
		   ($phonetbl eq "chris_rus_abs")) {
			$order = "asc";
		...
		if($phonetbl =~ /russ/) {
			...
			$sql = "select distinct(p1.voter_id),mvf.birth_date from $phonetbl p1, mvf where (p1.voter_id=mvf.voter_id) and (p1.stat=2) and p1.voter_id not in ( select voter_id from canv where ( ( cmpid = $CMP ) and ( sup <= 5 ) ) ) order by mvf.birth_date $order  limit 1";
		}
		my $sth = $conn->exec($sql);
		my(@row) = $sth->fetchrow;
		my $voter_id = $row[0];
...
<FORM ACTION='http://cybre.net/elect/chris/phone.pl' METHOD='GET'>
<INPUT TYPE='HIDDEN' NAME='voter_id' VALUE=$voter_id>
<INPUT TYPE='HIDDEN' NAME='cmpid' VALUE=$CMP>
...
my $sql = "select distinct(id),tag,title,phone.cmpid,campaign from lists phone,cmp where phone.cmpid=cmp.cmpid and cmp.cmpid=$CMP";
my $sth = $conn->exec($sql);
