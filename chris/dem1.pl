...
if($ENV{QUERY_STRING} || ($ENV{REQUEST_METHOD} eq "POST")) {
	...
	if($#nhists > -1) {
		my(@NH);
		foreach $nhist(@nhists) {
			push(@NH,"( $nhist = 'N' ) ");
		}
		if($#bhists == 0) {
			push(@hwheres,$HW[0]);
		} else {
			push(@nhwheres," ( " . join(" $HISTBOOL ",@NH) . " ) ");
		}
	}
	if($#vhists > -1) {
		my(@VH);
		foreach $vhist(@vhists) {
			push(@VH,"( $vhist = 'V' ) ");
		}
		if($#vhists == 0) {
			push(@hwheres,$VH[0]);
		} else {
			push(@vhwheres," ( " . join(" $HISTBOOL ",@VH) . " ) ");
		}
	}
	if($#ahists > -1) {
		my(@AH);
		foreach $ahist(@ahists) {
			push(@AH,"( $ahist = 'A' ) ");
		}
		if($#hists == 0) {
			push(@hwheres,$AH[0]);
		} else {
			push(@ahwheres," ( " . join(" $HISTBOOL ",@AH) . " ) ");
		}
	}
	if($#bplaces > -1) {
		my(@b);
	...
	if($#inc_canvs > -1) {
		foreach $inc_canv(@inc_canvs) {
			if($inc_canv =~ /^[0-9]+$/) {
				if($inc_canv == 5) {
					push(@incc," ( (sup = $inc_canv ) and ( mvf$cut.voter_id=canv.voter_id and mvf$cut.phone=mvf$cut.voice) ) ");
				} else {
	...
	if($#exc_canvs > -1) {
		foreach $exc_canv(@exc_canvs) {
			if($exc_canv =~ /^[0-9]+$/) {
				if($exc_canv == 5) {
					push(@excc," ( (sup = $exc_canv ) and ( mvf$cut.voter_id=canv.voter_id and mvf$cut.phone=mvf$cut.voice) ) ");
				} else {
	...
	if($#owheres >= 0) {
		if(length($where)) {
			...
			$where = " ( " . join(" OR ",@owheres) .  " ) ";
		}
	}
	if($#excc > -1) {
		push(@cwheres,"mvf$cut.voter_id not in ( select distinct(voter_id) from canv where cmpid=19 and ( " . join(" OR ",@excc) . " ) ) ");
	}
	if($#incc > -1) {
		push(@cwheres,"mvf$cut.voter_id in ( select distinct(voter_id) from canv where cmpid=19 and ( " . join(" OR ",@incc) . " ) ) ");
	}
	...
	$hwheres = join(" OR ",@ahwheres,@vhwheres,@nhwheres,@rdwheres);
	if(length($hwheres)) {
		if(length($where)) {
			$where .= " AND ( $hwheres ) ";
		} else {
...
print "<TABLE CELLPADDING=3 BORDER=1>\n";
foreach $o ( @{[ "desc" , "asc" ]} ) {
	$sql = "select pvi.pvi,pvi.precinct,count(distinct(mvf$cut.voter_id)) from mvf$cut where pvi.precinct in ( select distinct(precinct) from mvf$cut $w $and pvi.precinct=mvf$cut.precinct) and mvf$cut.precinct=pvi.precinct group by pvi.precinct,pvi.pvi order by pvi.pvi $o limit 5";
	print "<hr>$sql<hr>\n";
	$qy = $conn->exec($sql);
	if($o eq "desc") {
...
$opts = "<OPTION VALUE='M'>Mail\n<OPTION VALUE='P'>:hone\n<OPTION VALUE='W'>Walk\n<OPTION VALUE='E'>Email<OPTION VALUE='R'>RoboCall<OPTION VALUE='S'>Sweep";
