...
print "<TABLE CELLPADDING=3 BORDER=1>\n";
foreach $o ( @{[ "desc" , "asc" ]} ) {
	$sql = "select pvi,pvi.precinct,count(distinct($mvf.voter_id)) from $from,pvi where pvi.precinct in ( select distinct($mvf.precinct) from $from $w $and pvi.precinct=$mvf.precinct) and $mvf.precinct=pvi.precinct group by pvi.precinct,pvi.pvi order by pvi $o limit 5";
	$debug && print $sql;
	$qy = $conn->exec($sql);
	if($o eq "desc") {
