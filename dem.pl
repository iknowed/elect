...
if($ENV{QUERY_STRING} || ($ENV{REQUEST_METHOD} eq "POST")) {
	...
	foreach(split(/&/,$req)) {
		...
		if(/^pvimin/) {
			($foo,$pvimin) = split(/=/);
			$tables{pvi} = 1 if ($pvimin);
		}
		if(/^pvimax/) {
			($foo,$pvimax) = split(/=/);
			$tables{pvi} = 1 if($pvimax);
		}
		if(/^medmin/) {
			($foo,$medmin) = split(/=/);
		}
	...
	if($#langs > -1) {
		my(@l);
		foreach $lid (@langs) {
			push(@l,"(  $mvf.voter_id = lang.voter_id and lang.lid=$lid ) ");
		}
		if($#langs == 0) {
			push(@wheres," $l[0] ");
		} else {
			push(@wheres, join(" OR ",@l));
		}
	}
	if($#genders > -1) {
		my(@g);
	...
	if($#hoods > -1) {
		my(@s);
		foreach $hood(@hoods) {
			push(@s,"(  pct.hood=$hood and $mvf.precinct=pct.pct )");
		}
		if($#hoods== 0) {
			push(@owheres,$s[0]);
		} else {
			push(@owheres,join(" or ",@s));
		}
	}
	if($#congs > -1) {
		my(@s);
	...
	if($#eths > -1) {
		my(@e);
		foreach $eid(@eths) {
			push(@e,"(  $mvf.voter_id=eth.voter_id and eth.eid=$eid ) ");
		}
		if($#eths== 0) {
			push(@wheres,$e[0]);
		} else {
	...
	if($pvimin) {
		push(@wheres," ( $mvf.precinct=pvi.pct and pvi.pvi_new > $pvimin  ) ");
	}
	if($pvimax) {
		push(@wheres," ( $mvf.precinct=pvi.pct and pvi.pvi_new < $pvimax  ) ");
	}
	if($medmin) {
	...
	#map(s/voted = 1/$mvf.voted = 1/,@excc);
	#map(s/voted = 1/$mvf.voted = 1/,@incc);
	if($#excc > -1) {
		push(@cwheres,"$mvf.voter_id not in ( select voter_id from canv where cmpid=19 and ( " . join(" OR ",@excc) . " ) ) ");
	}
	if($#incc > -1) {
		push(@cwheres,"$mvf.voter_id in ( select voter_id from canv where cmpid=19 and ( " . join(" OR ",@incc) . " ) ) ");
	}
	...
	if($#cwheres > -1) {
		if(length($where)) {
			...
			$where = " ( $cwhere ) " if length $cwhere;
		}
	}
#$from =  $from ? $mvf .",".$from : $mvf;
select(STDOUT);
$| = 1;
...
$debug && print "<br>from: $from\n";
$w = "where ( $where ) " if (length($where));
$and = "and" if(length($where));
$sql = "select count(distinct($mvf.voter_id)) from $from $w ";
...
$w = "where ( $where ) " if (length($where));
$and = "and" if(length($where));
$sql = "select perm_category,count(distinct($mvf.voter_id)) from $from  $w group by perm_category order by count(distinct($mvf.voter_id)) desc";    
$qy = $conn->exec($sql);
...
$w = "where ( $where ) " if (length($where));
$sql = "select bplace,count(distinct($mvf.voter_id)) from $from,bplaces $w $and ( code=birth_place and birth_place not in ( select st from us )) group by bplace order by count(distinct($mvf.voter_id)) desc";
$qy = $conn->exec($sql);
...
$w = "where ( $where ) " if (length($where));
	$sql = "select ethnicities.ethnicity,count(distinct($mvf.voter_id)) from $from,ethnicities,eth $w $and eth.eid=ethnicities.eid and $mvf.voter_id=eth.voter_id group by ethnicities.ethnicity order by count(distinct($mvf.voter_id)) desc";
	print $sql."<br>\n";
	$qy = $conn->exec($sql);
	...
	$t{'lang'} = 1;
	my $f = join(",",keys %t);
	$sql = " select count(distinct($mvf.voter_id)),langs.language from $from $w  ( $mvf.voter_id=lang.voter_id and lang.lid=langs.lid ) group by langs.language order by count(distinct($mvf.voter_id)) desc";
print $sql;
$qy = $conn->exec($sql);
...
$w = "where ( $where ) " if (length($where));
$sql = "select count(distinct($mvf.voter_id)) from $from $w $and email is null";
#print "<TR><TD>$sql</TD></TR>\n";
$qy = $conn->exec($sql);
my(@r) = $qy->fetchrow;
my $nullemails = $r[0];
$sql = "select count(distinct($mvf.voter_id)) from $from $w $and email is not null";
#print "<TR><TD>$sql</TD></TR>\n";
$qy = $conn->exec($sql);
my(@r) = $qy->fetchrow;
my $emails = $r[0];
...
foreach $o ( @{[ "desc" , "asc" ]} ) {
	...
	$sql = "select npvi,pvi.pct,count(distinct($mvf.voter_id)) from $f where pvi.pct in ( select distinct($mvf.precinct) from $from $w $and pvi.pct=$mvf.precinct) and $mvf.precinct=pvi.pct group by pvi.pct,pvi.npvi order by pvi.npvi $o limit 5";
	$debug && print $sql;
	$qy = $conn->exec($sql);
	if($o eq "desc") {
...
$w = "where ( $where ) " if (length($where));
$sql = "select ten.tenant,count(distinct($mvf.voter_id)) from $from,ten $w $and ten.voter_id=$mvf.voter_id group by ten.tenant order by count(distinct($mvf.voter_id)) desc";
$debug && print "$sql\n";
$qy = $conn->exec($sql);
...
for($a = 18 ; $a <= 108 ; $a += 10 ) {
	...
	$w = "where ( $where ) " if (length($where));
	$and = " and "  if (length($where));
	$sql = "select count(distinct($mvf.voter_id)) from $from  $w $and ((now() - birth_date) >= '$a years' and ((now()-birth_date) < '$b years'))";
	$debug && print $sql;
	$qy = $conn->exec($sql);
	my(@row) = $qy->fetchrow;
	$n = $row[0];
...
for($a = 0 ; $a <= 40; $a += $i) {
	...
	$w = "where ( $where ) " if (length($where));
	$and = " and "  if (length($where));
	$sql = "select count(distinct($mvf.voter_id)) from $from  $w $and ((now() - reg_date) >= '$a years' and ((now()-reg_date) < '$b years'))";
	$debug && print $sql."<br>";
	$qy = $conn->exec($sql);
	my(@row) = $qy->fetchrow;
	$n = $row[0];
...
$w = "where ( $where ) " if (length($where));
$and = " and "  if (length($where));
$sql = "select age(birth_date) from $from  $w $and ( birth_date != '1900-01-01' ) order by age(birth_date) offset $med limit 1";
$debug && print "$sql\n";
$qy = $conn->exec($sql);
my(@row) = $qy->fetchrow;
$n = $row[0];
...
$sql = "select attname from pg_attribute where attrelid in ( select oid from pg_class where relname='voters' and attstattarget < 0)";
$field_sel = "<SELECT NAME='fields' MULTIPLE>";
$qy = $conn->exec($sql);
while(@row = $qy->fetchrow) {
$fld = $row[0];
$field_sel .= "<OPTION NAME='$fld'>$fld\n";
}
$field_sel .= "</SELECT>";

print <<EOF;
</TD></TR></TABLE>
<FORM ACTION='http://cybre.net/elect/do.pl' METHOD='GET'>
...
$sql = "select distinct(precinct),count(precinct) from $mvf group by precinct order by precinct asc";
$qy = $conn->exec($sql);
while(my(@r) = $qy->fetchrow) {
$r[0] =~ s/\ //g;
...
$sql = "select distinct(substr(zip,1,5)),count(substr(zip,1,5)) from $mvf group by substr(zip,1,5) order by substr(zip,1,5) asc";
$qy = $conn->exec($sql);
while(my(@r) = $qy->fetchrow) {
$r[0] =~ s/\ //g;
...
$hood = "<SELECT SIZE=2 NAME='hood' TITLE='Neighborhood Preciinct Groups' MULTIPLE>\n";
$sql = "select distinct(nid),hood from hoods";
$qy = $conn->exec($sql);
while(my(@r) = $qy->fetchrow) {
$hood .= "<OPTION VALUE=$r[0]>$r[1]\n" if(length $r[0]);
}
$hood .= "</SELECT>\n";
...
$sql = " select perm_category,count(perm_category) from $mvf group by perm_category order by count(perm_category) desc ";

$qy = $conn->exec($sql);
while(my(@r) = $qy->fetchrow) {
if(!length($r[0])) {
		$perm .= "<OPTION VALUE=\"$r[0]\">EDAY: $r[1]\n";
	} else {
		$perm .= "<OPTION VALUE=\"$r[0]\">$r[0]: $r[1]\n";
	}
}
$perm .= "</SELECT>\n";

my %es;
$hist1 =  "<p>Voting History\n";
...
while(my(@row) = $qy->fetchrow) {
	foreach(@row) {
		if($row[0] =~ /^e[09][\d]{5}/) {
			$e = $row[0];
			my($year,$month,$date) = ($e =~ m/^e(..)(..)(..)$/);
			if($year < 10) { 
				$year = "20$year";
			} else {
				$year = "19$year";
			}
			my($yr,$mo,$da) = extractDate($e);
			my $hist1 = "<br>$da $mo $year";
			$hist1 .= "<INPUT TYPE='CHECKBOX'  NAME='hist' VALUE='".$e.":V'>Eday";
			$hist1 .= "<INPUT TYPE='CHECKBOX'  NAME='hist' VALUE='".$e.":A'>VBM";
			$hist1 .= "<INPUT TYPE='CHECKBOX'  NAME='hist' VALUE='".$e.":N'>No";
			if($e eq 'e080408') {
				$hist1 .= "<font size='tiny'>CD 12 Special Election</font>"
			}
			$es{"$year$month$date"} = $hist1;
		}
	}
}

foreach ( reverse sort { $a <=> $b } keys %es ) {
	$hist1 .= "$es{$_}\n";
}
$hist =  "<p>Voting History\n";
$hist .= "<br>05 Feb 2008";
$hist .= "<INPUT TYPE='CHECKBOX' ' NAME='hist' VALUE='e080205:V'>Eday";
$hist .= "<INPUT TYPE='CHECKBOX'  NAME='hist' VALUE='e080205:A'>VBM";
$hist .= "<INPUT TYPE='CHECKBOX'  NAME='hist' VALUE='e080205:N'>No";
$hist .= "<br>06 Nov 2007";
$hist .= "<INPUT TYPE='CHECKBOX' ' NAME='hist' VALUE='e071106:V'>Eday";
$hist .= "<INPUT TYPE='CHECKBOX'  NAME='hist' VALUE='e071106:A'>VBM";
$hist .= "<INPUT TYPE='CHECKBOX'  NAME='hist' VALUE='e071106:N'>No";
$hist .= "<br>07 Nov 2006";
$hist .= "<INPUT TYPE='CHECKBOX' ' NAME='hist' VALUE='e061107:V'>Eday";
$hist .= "<INPUT TYPE='CHECKBOX'  NAME='hist' VALUE='e061107:A'>VBM";
$hist .= "<INPUT TYPE='CHECKBOX'  NAME='hist' VALUE='e061107:N'>No";
$hist .= "<br>06 Jun 2006";
...
<BODY>
<center>Voter File Demographic Search</center>
<center><i>place mouse over anything for explanation</i></center>
<FORM METHOD="GET" ACTION="/elect/dem.pl">
<TABLE>
<TR>
...
<TD>$hist</TD>
<TD ALIGN='TOP'>Voting<br>History<br>Boolean<br>$histbool</TD>
</TR>
<TR>
<TD COLSPAN=3>
...
<TD>Precinct<br>$precinct<br>Neighborhood: $hood<br>Zip 5<br>$zip</TD>
</TR>
</TABLE>
<br>
...
sub	extractDate	{
	my($field) = @_;
	my($yr,$mo,$da) = ($field =~ m/^e(..)(..)(..)$/);
	return($yr,$months[$mo-1],$da);	
}
