...
if(0){
	...
	for (my $a = $a0 ; $a <= $an ; $a+=2 ) {
		my $sql = "select count(voter_id) from mvf where street='$street' and house_number=$a $apt_num_sql ";
		my $qy = $db->do($sql);
		my (@row) = $qy->fetchrow();
		if($row[0] == 0) {
			$sql = "select voter_id from mvf where street='$street' and house_number=$a";
			$qy = $db->do($sql);
		} else {
			$sql = "select voter_id from mvf where street='$street' and house_number=$a $apt_num_sql ";
			$qy = $db->do($sql);
		}
		if($qy) {
