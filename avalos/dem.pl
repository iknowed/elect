...
if($ENV{QUERY_STRING} || ($ENV{REQUEST_METHOD} eq "POST")) {
	...
	if($#perms > -1) {
		...
		foreach $perm (@perms) {
			...
			if(length($perm)) {	
				...
				push(@perm,"(  perm_category is null )");
			}
		}
		if($#perm == 0) {
			push(@wheres,$perm[0]);
	...
	if($pvimin) {
		push(@wheres," ( $mvf.precinct=pvi.pct and pvi.pvi_2008 > $pvimin  ) ");
	}
	if($pvimax) {
		push(@wheres," ( $mvf.precinct=pvi.pct and pvi.pvi_2008 < $pvimax  ) ");
	}
	if($medmin) {
...
$sql = " select count(*) from voters where perm_category is null";

$qy = $conn->exec($sql);

my $r = $qy->fetchrow;

$perm .= "<OPTION VALUE=\"\">EDAY: $r\n";
...
my %es;
$hist1 =  "<TABLE><TR><TD COLSPAN='3'>Voting History</TD></TR>\n";
$hist1 .=  "<TR><TD>Election</TD><TD>Eday</TD><TD>VBM</TD><TD>DNV</TD></TR>\n";
...
$sql = "SELECT columns.column_name FROM information_schema.columns where columns.table_name='voters' and columns.column_name ~ 'e[0-9]+' order by columns.column_name ;";
$qy = $conn->exec($sql);
while(my(@row) = $qy->fetchrow) {
	foreach(@row) {
		#if($row[0] =~ /^e[09][\d]{5}/) {
			...
			my $histline = "<TR><TD>$da $mo $year</TD>";
			$histline .= "<TD><INPUT TYPE='CHECKBOX'  NAME='hist' VALUE='".$e.":V'>Eday</TD>";
			$histline .= "<TD><INPUT TYPE='CHECKBOX'  NAME='hist' VALUE='".$e.":A'>VBM</TD>";
			$histline .= "<TD><INPUT TYPE='CHECKBOX'  NAME='hist' VALUE='".$e.":N'>No</TD>";
			if($e eq 'e080408') {
