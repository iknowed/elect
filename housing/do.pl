...
$f = "$from,lang,langs";
$sql = "select distinct($mvf.voter_id),phone,street,type,house_number,apartment_number,now()-birth_date,birth_place,gender,party,name_prefix,name_last,name_first,gender,perm_category,e060606,e051108,e041102,e040302,e031209,precinct,langs.language from $f $where and lang.lid=langs.lid and $mvf.voter_id=lang.voter_id order by precinct,phone";
open(FOO,">>/tmp/foo1");
print FOO "\n$sql\n";
close FOO;
my $qy = $conn->exec($sql);
...
my %voters,%precincts;
$sql = "select phone,street, type, house_number, $mvf.voter_id, apartment_number, now()-birth_date,birth_place, gender, party, name_prefix,name_last,name_first,gender,perm_category, e060606, e051108, e041102, e040302, e031209, precinct from $from $where order by precinct asc, street, house_number";

if(!length($pvi)) {
	my $sql = "select pvi from pvi where precinct=$pct";
	$qy = $conn->exec($sql);
	(@r) = $qy->fetchrow;
	$pvi = $r[0];
}
...
sub	mail	{
	...
	$q = $conn->exec($qc);
	(@r) = $q->fetchrow;
	$qy = "select distinct(voter_id),name_prefix,name_first,name_last,house_number,street,type,apartment_number,pre_dir,house_fraction,zip from $from $where order by street,type,house_number,apartment_number,zip";
	$q = $conn->exec($qy);
	my(%households);
	$file = ">/tmp/$pid";
	open(OUT,$file);
