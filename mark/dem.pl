...
if($ENV{QUERY_STRING} || ($ENV{REQUEST_METHOD} eq "POST")) {
	...
	if($#langs > -1) {
		my(@l);
		foreach $lid (@langs) {
			push(@l,"(  $mvf.voter_id in ( select voter_id from lang where lang.lid=$lid ) )");
		}
		if($#langs == 0) {
			push(@owheres," $l[0] ");
		} else {
	...
	if($pvimin) {
		push(@wheres," ( $mvf.precinct=pvi.pct and pvi.npvi > $pvimin  ) ");
	}
	if($pvimax) {
		push(@wheres," ( $mvf.precinct=pvi.pct and pvi.npvi < $pvimax  ) ");
	}
	if($medmin) {
...
$sql = " select count(distinct($mvf.voter_id)),langs.language from $from,langs,lang $w  ( $mvf.voter_id=lang.voter_id and lang.lid=langs.lid ) group by langs.language order by count(distinct($mvf.voter_id)) desc";
print $sql;
$qy = $conn->exec($sql);
...
<FORM ACTION='http://cybre.net/test.jah/do.pl' METHOD='GET'>
